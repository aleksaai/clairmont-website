...
const createSignedFileLinks = async (formData: any): Promise<FileLink[]> => {
  ...
  if (formData.taxCertificatesByYear) {
    Object.values(formData.taxCertificatesByYear).forEach((files) =>
      extractPaths(files, filePaths)
    );
  }
  ...
  extractPaths(formData.alimonyProof, filePaths);
  ...
  for (const filePath of filePaths) {
    ...
    if (error || !data?.signedUrl) {
      throw new Error(`Dokumentlink fehlt: ${filePath}`);
    }
  ...
  console.log(
    `Prepared ${links.length} signed document links out of ${filePaths.size} files`,
  );
  return links;
};
      ...
      .map(
        (insurance) => `
              <tr>
                ...
                <td>${
          escapeHtml(formatCurrency(insurance?.yearlyContribution))
        }</td>
              </tr>
            `,
      )
      ...
      .map(
        (property, index) => `
              <tr>
                ...
                <td>${
          escapeHtml(formatCurrency(property?.additionalCosts))
        }</td>
                <td>${
          escapeHtml(formatCurrency(property?.interestExpense))
        }</td>
          ...
          escapeHtml(
            property?.otherCostsDescription
              ? `${formatValue(property.otherCostsDescription)}: ${
                formatCurrency(property.otherCostsAmount)
              }`
              : "N/A",
          )
        }</td>
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const {
      ...
      expectedFileCount,
      testMode = false,
    }: PrognoseEmailRequest = await req.json();
    if (
      !submissionId || !Number.isInteger(expectedFileCount) ||
      expectedFileCount < 1
    ) {
      return new Response(
        JSON.stringify({ error: "Invalid document manifest" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json", ...corsHeaders },
        },
    ...
    const recipient = testMode
      ? "info@aleksa.ai"
      : "service@clairmont-advisory.com";
    console.log(`Sending prognose email to ${recipient}`);

    const fileLinks = await createSignedFileLinks(formData);
    if (fileLinks.length !== expectedFileCount) {
      throw new Error(
        `E-Mail-Dokumentprüfung fehlgeschlagen: ${fileLinks.length} von ${expectedFileCount} Links verfügbar.`,
      );
    }
    const taxCertificateCount = Object.values(
      formData.taxCertificatesByYear ?? {},
    ).reduce(
    ...
    const emailResponse = await resend.emails.send({
      ...
      to: [recipient],
      replyTo: userEmail,
      subject: `${testMode ? "[SYSTEM TEST] " : ""}Neue Steuerprognose von ${
      ...
      html: `
        ...
        renderKeyValueTable([
          ...
          [
            "Abweichende Adresse auf Lohnsteuerbescheid",
            formData.differentAddress,
          ],
          [
            "Alternative Adresse",
            formData.differentAddress ? formData.alternativeAddress : "N/A",
          ],
        ])
      }
        ...
        renderKeyValueTable([
          ...
          [
            ...
            formData.workplace
              ? `${formData.workplace.street || ""}, ${
                formData.workplace.zipCode || ""
              } ${formData.workplace.city || ""}`.trim()
              : "N/A",
          ],
          ...
          [
            "Arbeitszeiträume / Lücken",
            formData.workPeriodsByYear
              ? JSON.stringify(formData.workPeriodsByYear)
              : "N/A",
          ],
        ])
      }
        ...
        renderKeyValueTable([
          ...
          [
            "Staatliche Leistungen Summe",
            formatCurrency(formData.socialBenefitAmount),
          ],
          [
            "Steuerbescheide vorhanden für",
            Array.isArray(formData.taxYears)
              ? formData.taxYears.join(", ")
              : "N/A",
          ],
        ])
      }
        ...
        renderKeyValueTable([
          ...
          [
            "Sonstige Mitgliedschaften Details",
            formData.otherMembershipsDetails,
          ],
        ])
      }
        ...
        renderKeyValueTable([
          ...
          [
            "Behindertenausweis",
            getArrayLength(formData.documents?.disabilityCertificate),
          ],
          ["Immobilien-Unterlagen", getArrayLength(formData.propertyDocuments)],
          [
            "Crypto-/Trading-Unterlagen",
            getArrayLength(formData.cryptoDocuments),
          ],
          [
            "Fortbildungskosten-Nachweise",
            getArrayLength(formData.trainingCostDocuments),
          ],
          [
            "Arbeitsmittel-Nachweise",
            getArrayLength(formData.businessEquipmentDocuments),
          ],
          [
            "Selbstständigkeit-Unterlagen",
            getArrayLength(formData.businessDocuments),
          ],
          ["KFZ-Unterlagen", getArrayLength(formData.vehicleDocuments)],
          [
            "Ausbildungs-/Studiumsunterlagen",
            getArrayLength(formData.educationDocuments),
          ],
          [
            "Ehepartner Einkommensunterlagen",
            getArrayLength(formData.spouseIncomeDocuments),
          ],
          [
            "Ehepartner Elterngeldnachweise",
            getArrayLength(formData.spouseParentalBenefitDocuments),
          ],
          ["Unterhaltsnachweise", getArrayLength(formData.alimonyProof)],
          ["Weitere Unterlagen", getArrayLength(formData.additionalDocuments)],
          [
            "Sonstige Belege",
            getArrayLength(formData.documents?.otherDocuments),
          ],
    ...
    if (emailResponse.error || !emailResponse.data?.id) {
      throw new Error(
        `Resend hat den Versand abgelehnt: ${
          emailResponse.error?.message || "keine Versand-ID"
        }`,
      );
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        document_count: fileLinks.length,
      }),
      {
        status: 200,
        headers: {
