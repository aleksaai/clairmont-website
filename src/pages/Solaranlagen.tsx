...
import { ArrowRight, Sun, BarChart3, Users, FileCheck, Wrench } from "lucide-react";
import { useNavigate } from "react-router-dom";
import Navigation from "@/components/Navigation";
import Footer from "@/components/Footer";
import heroImage from "@/assets/services/solar-hero.jpg";

const Solaranlagen = () => {
  const navigate = useNavigate();

  const services = [
    { icon: Sun, title: "Investitionsberatung", desc: "Beratung zu Investitionen in Solaranlagen und Wärmepumpen." },
    { icon: BarChart3, title: "Wirtschaftlichkeitsanalyse", desc: "Wirtschaftlichkeitsanalyse und Renditeberechnung für Energieprojekte." },
    { icon: Users, title: "Partnervermittlung", desc: "Vermittlung von qualifizierten Installationspartnern." },
    { icon: FileCheck, title: "Fördermittel", desc: "Unterstützung bei Fördermittelanträgen und staatlichen Zuschüssen." },
    { icon: Wrench, title: "Rundum-Begleitung", desc: "Begleitung von der Planung bis zur Inbetriebnahme." },
  ];
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-32 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center justify-center px-4 py-1.5 border border-white/20 rounded-full mb-6">
              <span className="text-sm font-medium text-[hsl(var(--glass-text))]">Erneuerbare Energien</span>
            </div>
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-light text-[hsl(var(--glass-text))] mb-6 leading-tight">
              Solaranlagen &<br />Wärmepumpen
            </h1>
            <p className="text-lg md:text-xl text-[hsl(var(--glass-text))]/80 font-light max-w-2xl mx-auto mb-10">
              Beratung und Investitionen in erneuerbare Energielösungen — nachhaltig, wirtschaftlich, zukunftssicher.
            </p>
            <Button size="lg" className="rounded-full px-10 h-14 text-lg" onClick={() => navigate("/kontakt?service=solaranlagen")}>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="bg-primary/5 rounded-3xl p-10 border border-primary/10">
            ...
            <p className="text-muted-foreground font-light leading-relaxed">
              Immobilienbesitzer, Unternehmen und Investoren, die in erneuerbare Energien investieren und von staatlichen Förderungen profitieren möchten.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 bg-primary/[0.03]">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-4xl font-light text-primary mb-6">In die Zukunft investieren?</h2>
