...
import { ArrowRight, Scale, FileText, Building, Globe } from "lucide-react";
import { useNavigate } from "react-router-dom";
import Navigation from "@/components/Navigation";
import Footer from "@/components/Footer";
import heroImage from "@/assets/services/rechtsberatung-hero.jpg";
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-32 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center justify-center px-4 py-1.5 border border-white/20 rounded-full mb-6">
              <span className="text-sm font-medium text-[hsl(var(--glass-text))]">Legal</span>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto font-light">
              Wir verbinden Sie mit den besten Rechtsexperten für Ihre individuelle Situation.
            </p>
          </motion.div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: Scale, title: "Wirtschaftsrecht", desc: "Beratung bei Unternehmensgründung, Gesellschaftsrecht und geschäftlichen Transaktionen." },
              { icon: FileText, title: "Vertragsrecht", desc: "Prüfung, Erstellung und Verhandlung von Verträgen aller Art." },
              { icon: Building, title: "Immobilienrecht", desc: "Rechtliche Begleitung bei Immobilientransaktionen im In- und Ausland." },
              { icon: Globe, title: "Internationales Recht", desc: "Grenzüberschreitende Rechtsberatung für internationale Geschäftstätigkeiten." },
            ].map((item, index) => (
              <motion.div
                key={item.title}
                initial={{ opacity: 0, y: 20 }}
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-light text-primary mb-6">
              Rechtliche Fragen? Wir helfen weiter.
            </h2>
